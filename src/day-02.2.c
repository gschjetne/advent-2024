...
int is_safe_lax(vector *report) {
    int result = is_safe(report);
    ...
    size_t length = vector_length(report);
    vector copy;
    vector_init(&copy, report->size);

    for (int i = 0; i < length; i++) {
        vector_copy(&copy, report);
        vector_delete(&copy, i);
        result = is_safe(&copy);
